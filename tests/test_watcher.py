...
from mailagent.watcher import run
...
def test_unknown_watch_descriptor_ignored(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "mailagent.watcher._create_notifier", lambda: (notifier, object())
    )
    monkeypatch.setattr(
        "mailagent.watcher.maildir_new_path", lambda _address: watch_dir
    )
    monkeypatch.setattr(
        "mailagent.watcher.build_provider", lambda _config, _name: object()
    )
    ...
    monkeypatch.setattr("mailagent.watcher.process_email", fake_process)
