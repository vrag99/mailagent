...
async def create_provider(request: Request, name: str, body: ProviderRequest) -> ProviderResponse:
    cm = _get_cm(request)

    valid_types = {"openai", "anthropic", "gemini", "openrouter", "groq"}
    if body.type not in valid_types:
        raise HTTPException(status_code=400, detail=f"Invalid provider type: {body.type}. Must be one of: {', '.join(sorted(valid_types))}")
...
async def update_provider(request: Request, name: str, body: ProviderRequest) -> ProviderResponse:
    cm = _get_cm(request)
