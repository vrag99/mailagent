...
def _inbox_to_response(inbox: InboxConfig) -> InboxResponse:
    ...
    for w in inbox.workflows:
        kw = None
        if w.match.keywords:
            from ..models import KeywordMatchRequest
...
def _workflow_request_to_model(wr) -> Workflow:
    keywords = None
...
async def create_inbox(request: Request, body: InboxRequest) -> InboxResponse:
    ...
    # Provision mailbox in docker-mailserver if provisioner available
    provisioner = _get_provisioner(request)
    if provisioner:
        try:
            provisioner.add_account(body.address, body.password)
        except Exception as exc:
            raise HTTPException(status_code=500, detail=f"Failed to provision mailbox: {exc}")
...
async def update_inbox(request: Request, address: str, body: InboxUpdateRequest) -> InboxResponse:
    cm = _get_cm(request)
    inbox = cm.get_inbox(address)
    if inbox is None:
        raise HTTPException(status_code=404, detail=f"Inbox not found: {address}")
    ...
    if body.classify_provider is not None:
        if body.classify_provider not in config.providers:
            raise HTTPException(status_code=400, detail=f"Unknown classify_provider: {body.classify_provider}")
    ...
    if body.reply_provider is not None:
        if body.reply_provider not in config.providers:
            raise HTTPException(status_code=400, detail=f"Unknown reply_provider: {body.reply_provider}")
